...
```
所以，关键就是利用安全输入的字符串经过替换操作后的**长度膨胀**，实现 strcpy 的溢出。同时`s` 在栈上的起始位置距离返回地址（ret）的字节数是**64 字节**（这是从反编译信息得出的：`s` 在 `ebp-0x3C`，即从 `s` 开始到 `ebp` 共有 0x3C = 60 字节，再加 4 字节的 saved EBP -> 到返回地址为 64）。


所以我们只要放入带I的小于32字节的payload然后经过膨胀后即可实现栈溢出替换.
